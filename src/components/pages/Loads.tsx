...
import { Search, Filter, Plus, Calendar, MapPin, User, Clock } from 'lucide-react';
...
export function Loads() {
  ...
  const filteredLoads = loads.filter(load => {
    ...
    const matchesSearch = load.id.toLowerCase().includes(searchTerm.toLowerCase()) ||
      ...
      load.dropOffLocation.city.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    const matchesDriver = driverFilter === 'all' || load.driverId === driverFilter;
      ...
      <Card className="bg-custom-surface border-custom-border">
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-custom-text-secondary">Driver</label>
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-custom-border">
                <TableHead className="text-custom-text-secondary">Load ID</TableHead>
                <TableHead className="text-custom-text-secondary">Route</TableHead>
                <TableHead className="text-custom-text-secondary">Status</TableHead>
                <TableHead className="text-custom-text-secondary">Driver</TableHead>
                <TableHead className="text-custom-text-secondary">Pickup Date</TableHead>
                <TableHead className="text-custom-text-secondary">Delivery Date</TableHead>
                <TableHead className="text-custom-text-secondary">Rate</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {paginatedLoads.map((load) => {
                const driver = load.driverId ? getDriverById(load.driverId) : null;
                return (
                  <TableRow key={load.id} className="border-custom-border hover:bg-custom-surface-hover">
                    <TableCell className="font-medium text-custom-text-primary text-start">{load.externalLoadId || load.id}</TableCell>
                    <TableCell>
                      <div className="space-y-1">
                        <div className="flex items-center gap-1 text-custom-text-primary">
                          <MapPin className="h-3 w-3 text-custom-success" />
                          <span className="text-sm">{formatLocation(load.pickUpLocation)}</span>
                        </div>
                        <div className="flex items-center gap-1 text-custom-text-primary">
                          <MapPin className="h-3 w-3 text-custom-error" />
                          <span className="text-sm">{formatLocation(load.dropOffLocation)}</span>
                        </div>
                      </div>
                    </TableCell>
                    ...
                    <TableCell>
                      {driver ? (
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-custom-text-primary">{driver.name}</span>
                        </div>
                      ) : (
                        <span className="text-custom-text-disabled">Unassigned</span>
                      )}
                    </TableCell>
                    <TableCell>
                      <div className="flex items-center gap-2">
                        <Clock className="h-4 w-4 text-custom-text-secondary" />
                        <span className="text-custom-text-primary">{formatDate(load.pickUpDate)}</span>
                      </div>
                    </TableCell>
                    <TableCell>
                      <div className="flex items-center gap-2">
                        <Clock className="h-4 w-4 text-custom-text-secondary" />
                        <span className="text-custom-text-primary">{formatDate(load.deliveryDate)}</span>
                      </div>
                    </TableCell>
