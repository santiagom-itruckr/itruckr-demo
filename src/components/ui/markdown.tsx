...
const markdownComponents = {
  ...
  li: ({ children }: any) => (
    <li className="text-sm pl-3">{children}</li>
  ),
  table: ({ children }: any) => (
    <div className="border border-absolute-black rounded-md overflow-hidden">
      <table className="w-full border-collapse scale-[1.01]">
        {children}
      </table>
    </div>
  ),
  ...
  tr: ({ children }: any) => (
    <tr className="border border-absolute-black last-of-type rounded-b-md">{children}</tr>
  ),
  th: ({ children }: any) => (
